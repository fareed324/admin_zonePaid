...
try
{
		$x = " SELECT * FROM advert WHERE productfeed='1' AND status='1' AND last_update < '$check_age' AND lge < '1' ORDER BY last_update DESC LIMIT 1 ";
					...
					if ($zip)
					{
						while ($zip_entry = zip_read($zip))
						{
							try
							{
								...
								if ($new_filesize > "4")
								{
									$logText.= " File size greater then 4MB, should be run by another process ";
													
									$ud = " UPDATE advert SET lge='1', feed_size='$new_filesize' WHERE id='$advertid' ";
								...
								else if ($new_filesize == "0")
								{
									...
									$ud = " UPDATE advert SET productfeed='0', feed_size='$new_filesize' WHERE id='$advertid' ";
									$udr = mysqli_query($conn,$ud);																
								}
								else
								{
									$ud = " UPDATE advert SET last_update='$timestamp', feed_size='$new_filesize' WHERE id='$advertid' ";
									$udr = mysqli_query($conn,$ud);	
