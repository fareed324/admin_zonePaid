...
if ($act == "1"){
	$q3 = "INSERT INTO cats (name, maincatid, link) VALUES('$name','$main_cat_id', '$urllink') ";
	$qa3 = mysqli_query($conn,$q3);
	$type = "3";
	$mess = "Added New User Sub Cat";
	
}
// ACT=2 Delete from Database
if ($act == "2"){
	$q4 = "DELETE FROM cats WHERE id='$sub_cat_id'";
	$qa4 = mysqli_query($conn,$q4);
	$type = "1";
	$mess = "Deleted User Sub Cat ID:".$sub_cat_id;
}

// ACT=3 Modify Data

if ($act == "3"){
	$q5 = "UPDATE cats SET name='$sub_cat_name', maincatid='$main_cat_id', link='$urllink' WHERE id='$sub_cat_id'";
	$qa5 = mysqli_query($conn,$q5);
	$type = "3";
	$mess = "UPDATED User Sub Cat ID:".$sub_cat_id;
...
?>
		...
		<div id="content" class="box">
			...
			<form name="newadminsubcat" method="post" action="usersubcat.php?act=1">
						<fieldset>
				<legend>Add New Sub Cat For User Area</legend>
				<table class="nostyle">
					<tr>
			...
			<?php
				$colcount = '0';
				// get all the data
				$q = "SELECT * FROM cats WHERE active = 1 ORDER BY name ASC";
				...
				while($qarow = mysqli_fetch_array($qa)){
					...
					//get main cat name
					$q2 = "SELECT * FROM maincat WHERE id='$main_cat_id'";
					...
					$urllink = $qarow['link'];
					echo '
					<form name="editsubCat" method="post" action="usersubcat.php?act=3&sub_cat_id='.$sub_cat_id.'">
					<tr'.table_background($colcount).'>
						...
						$q5 = "SELECT * FROM maincat ORDER BY name";
						$qa5 = mysqli_query($conn,$q5);
						...
						<td align="center" valign="middle"><a href="usersubcat.php?act=2&sub_cat_id='.$sub_cat_id.'"><img src="gfx/Close-2-icon.jpg" width="24" height="24" alt=""/></a></td>
					</tr>
					</form>					
					';
				}
