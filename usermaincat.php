...
$page_title = "Edit / Add User Main Cat";
...
if ($act == "1"){
	$q3 = "INSERT INTO maincat (name) VALUES('$main_cat_name') ";
	$qa3 = mysqli_query($conn,$q3);
	$type = "3";
	$mess = "Added New User Main Cat";
	
}

// ACT=2 Delete from Database
if ($act == "2"){
	$q4 = "DELETE FROM maincat WHERE id='$main_cat_id'";
	$qa4 = mysqli_query($conn,$q4);
	$type = "1";
	$mess = "Deleted User Main Cat ID:".$main_cat_id;
}
// ACT=3 Modify Data

if ($act == "3"){
	$q5 = "UPDATE maincat SET name='$main_cat_name' WHERE id='$main_cat_id'";
	$qa5 = mysqli_query($conn,$q5);
	$type = "3";
	$mess = "UPDATED User Main Cat ID:".$main_cat_id." To: ".$main_cat_name;
...
?>
		...
		<div id="content" class="box">
		  ...
		  <form name="newadmincat" method="post" action="usermaincat.php?act=1">
			<fieldset>
				<legend>Add New Main Cat For User Area</legend>
				<table class="nostyle">
					<tr>
					...
					<tr>
						...
						<!-- Table (TABLE) -->
		  <h3 class="tit">List Of Current Main User Cats</h3>
			<table>
				<tr>
				...
				$colcount = '0';
				// get all the data
				$q = "SELECT * FROM maincat ORDER BY name";
				...
				while($qarow = mysqli_fetch_array($qa)){
					...
					$q2 = "SELECT * FROM cats WHERE maincatid='".$main_cat_id."'";
					$qa2 = mysqli_query($conn,$q2);
					$qn2 = mysqli_num_rows($qa2);
					echo '
					<form name="editMainCat" method="post" action="usermaincat.php?act=3&main_cat_id='.$main_cat_id.'">
							<tr'.table_background($colcount).'>
								...
								<td align="center"><a href="usermaincat.php?act=2&main_cat_id='.$main_cat_id.'"><img src="gfx/Close-2-icon.jpg" width="24" height="24" alt=""/></a></td>
							</tr>	
					</form>
					';
				}
